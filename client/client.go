...
// GetClient open rpc connection to LND daemon
...
func getClientConn(ctx *cli.Context, skipMacaroons bool) *grpc.ClientConn {
	...
	if !ctx.Bool("no-macaroons") && !skipMacaroons {
		...
		macConstraints := []macaroons.Constraint{
			...
			macaroons.TimeoutConstraint(ctx.Int64("macaroontimeout")),

			// Lock macaroon down to a specific IP address.
			macaroons.IPLockConstraint(ctx.String("macaroonip")),
...
func fatal(err error) {
	fmt.Fprintf(os.Stderr, "[lnd client] %v\n", err)
	os.Exit(1)
}
